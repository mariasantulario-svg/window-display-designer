...
export function WindowDisplay({ festivity, placedElements, allElements, onRemoveElement, onUpdateElement }: WindowDisplayProps) {
  ...
  return (
    <div
      ref={canvasRef}
      className="relative w-full overflow-visible border-4 rounded-xl bg-white"
      style={{
        aspectRatio: "600/400",
        borderColor: festivity.color,
      }}
    ...
    >
      <svg viewBox="0 0 600 400" className="w-full h-full pointer-events-none">
        ...
        <text x="300" y="55" textAnchor="middle" fontFamily="'Architects Daughter', cursive" fontSize="24" fill="#333" fontWeight="bold" data-testid="text-festivity-name">
          {festivity.name}
        </text>
        <ShelfBooks />
        <ShelfStructure />
      ...
      {placedElements.map((placed, index) => {
        ...
        return (
          ...
          >
            <div className={`${isSelected ? "ring-2 ring-blue-500 ring-offset-2 rounded-2xl" : ""}`}>
              <StickerIcon iconName={element.iconName} color={displayColor} size={40} />
            </div>
                  ...
                  {festivity.colorPalette.map((c) => (
                    <button
                      ...
                      className={`w-4 h-4 rounded-full border-2 transition-transform ${
                        displayColor === c ? "border-slate-800 scale-125" : "border-transparent hover:scale-110"
                      }`}
